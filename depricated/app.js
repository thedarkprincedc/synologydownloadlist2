	...
	//	});
		//var mapsObject = googleMaps(layout.components.layout.cells("b"));
	//	var downloadList = download_list(layout.components.tabbar.cells("a1"));
		/*layout.components.tabbar.attachEvent("onSelect", function(){
			debugger;
	...
	//return app;
	/*return (function(app){
		$.get("config.json").then(function(msg){
			app.resource_host = msg.resources.host + "/";
			return $.get(app.resource_host + "./php/downloadviewerapp.php?action=getlogintoken", function(data) {
				app.synokey = (data.success) ? data.data.sid : "";
			});
		}).then(function(msg){
			var mapsObject = googleMaps(layout.components.layout.cells("b"));
			var downloadList = download_list(layout.components.tabbar.cells("a1"));
			...
			//downloadList.parse(taskList, "json");
		});
     })(app);*/
	/*
		getDownloads : function(){
			return $.get("./php/downloadviewerapp.php?action=getdownloads", {token : app.token});
		},
		init : function(){


			//app.downloadTabbar.onSelectTab("a1");
			/*app.downloadTabbar = component.attachList({
			...
			app.getLoginToken().then(function(msg){
				app.mainLayout.progressOff();
				setInterval(app.getDownloads(), 5000);
			});
		}
	};
